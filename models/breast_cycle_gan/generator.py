...
from models.utils.icnr import ICNR
...
def cyclegan_upsample(net, num_outputs, stride, method='conv2d_transpose', kernel_size=[3, 3], use_icnr=False):
  ...
  """
    with tf.variable_scope('upconv'):
        ...
        if use_icnr:
            print("using icnr")
            initializer = ICNR(layers.xavier_initializer(), scale=2)
        else:
            initializer = layers.xavier_initializer()

        if method == 'nn_upsample_conv':
            print("Upsampling method: nn_upsample_conv")
            # net = tf.Print(net, [tf.shape(net)], message="before resize", summarize=20)
            net = tf.image.resize_nearest_neighbor(net, [stride[0] * height, stride[1] * width])
            # net = tf.Print(net, [tf.shape(net)], message="after resize", summarize=20)
            net = tf.pad(net, spatial_pad_1, 'REFLECT')
            # net = tf.Print(net, [tf.shape(net)], message="after padding", summarize=20)
            net = convolution2d(net, num_outputs, kernel_size=[3, 3], padding='valid', weights_initializer=initializer)
            # net = tf.Print(net, [tf.shape(net)], message="after convolution", summarize=20)
        elif method == 'bilinear_upsample_conv':
            print("Upsampling method: bilinear_upsample_conv")
            net = tf.image.resize_bilinear(net, [stride[0] * height, stride[1] * width])
            net = tf.pad(net, spatial_pad_1, 'REFLECT')
            net = convolution2d(net, num_outputs, kernel_size=[3, 3], padding='valid', weights_initializer=initializer)
        elif method == 'conv2d_transpose':
            print("Upsampling method: conv2d_transpose")
            # net = tf.Print(net, [tf.shape(net)], message="before transpose_conv", summarize=20)
            ...
            # net = tf.Print(net, [tf.shape(net)], message="after transpose_conv", summarize=20)
        else:
...
def cyclegan_generator_resnet(images,
  ...
  Args:
    images: Input image tensor of shape [batch_size, h, w, 1].
    ...
    kernel_size: Size w or list/tuple [h, w] of the filter kernels for all inner
      layers.
    num_outputs: Number of output layers. Defaults to 1 for RGB.
  ...
  """
    ...
    with tf.contrib.framework.arg_scope(arg_scope_fn()):
        ...
        with tf.variable_scope('input'):
            ...
            net = convolution2d(net, num_filters, kernel_size=[7, 7], padding='VALID')
        ...
        with tf.variable_scope('encoder'):
            with tf.contrib.framework.arg_scope([convolution2d],
                ...
                net = convolution2d(net, num_filters * 2)
                end_points['encoder_1'] = net
                net = tf.pad(net, paddings, 'REFLECT')
                net = convolution2d(net, num_filters * 4)
            ...
            with tf.contrib.framework.arg_scope([convolution2d],
                ...
                for block_id in xrange(num_resnet_blocks):
                    with tf.variable_scope('block_{}'.format(block_id)):
                        ...
                        res_net = convolution2d(res_net, num_filters * 4)
                        res_net = tf.pad(res_net, paddings, 'REFLECT')
                        res_net = convolution2d(res_net, num_filters * 4, activation_fn=None)
                        net += res_net
            ...
            with tf.contrib.framework.arg_scope([convolution2d],
                ...
                with tf.variable_scope('decoder2'):
                    net = upsample_fn(
                            ...
                            method=upsample_method)
                    if self_attention:
                        print("Shape before self attention2: ", net.get_shape())
                        net = convolution2d(
                                net,
                                num_filters,
                                kernel_size=[1, 1],
                                ...
                                self_attention=True,
                                padding='VALID')
                        print("Shape after self attention2: ", net.get_shape())
                end_points['decoder2'] = net

        with tf.variable_scope('output'):
            net = tf.pad(net, spatial_pad_3, 'REFLECT')
            logits = convolution2d(net, num_outputs, [7, 7], activation_fn=None, normalizer_fn=None, padding='valid')
...
def generator(input_images,
  ...
  """
    ...
    with tf.contrib.framework.arg_scope(
            cyclegan_arg_scope(use_spectral_norm=use_spectral_norm, is_training=is_training)):
        output_images, _ = cyclegan_generator_resnet(
                input_images,
                arg_scope_fn=functools.partial(
                        cyclegan_arg_scope, use_spectral_norm=use_spectral_norm, is_training=is_training),
