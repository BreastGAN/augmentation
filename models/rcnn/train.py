...
from models.rcnn.basemodel import (image_preprocess, resnet_c4_backbone, resnet_conv5, resnet_fpn_backbone)

import models.rcnn.model_frcnn as model_frcnn
import models.rcnn.model_mrcnn as model_mrcnn
from models.rcnn.model_frcnn import (sample_fast_rcnn_targets, fastrcnn_outputs, fastrcnn_predictions, BoxProposals,
                                     FastRCNNHead)
from models.rcnn.model_mrcnn import maskrcnn_upXconv_head, maskrcnn_loss
from models.rcnn.model_rpn import rpn_head, rpn_losses, generate_rpn_proposals
from models.rcnn.model_fpn import (fpn_model, multilevel_roi_align, multilevel_rpn_losses, generate_fpn_proposals)
from models.rcnn.model_cascade import CascadeRCNNHead
from models.rcnn.model_box import (clip_boxes, crop_and_resize, roi_align, RPNAnchors)

from models.rcnn.data import (get_train_dataflow, get_eval_dataflow, get_all_anchors, get_all_anchors_fpn)
from models.rcnn.viz import (draw_annotation, draw_proposal_recall, draw_predictions, draw_final_outputs)
from models.rcnn.eval import (eval_coco, detect_one_image, print_evaluation_scores, DetectionResult)
from models.rcnn.config import finalize_configs, config as cfg
...
class ResNetC4Model(DetectionModel):
    ...
    def inputs(self):
        ...
        if cfg.MODE_MASK:
            ret.append(tf.placeholder(tf.uint8, (None, None, None), 'gt_masks'))  # NR_GT x height x width
        return ret
    ...
    def build_graph(self, *inputs):
        ...
        proposal_boxes, proposal_scores = generate_rpn_proposals(
                tf.reshape(pred_boxes_decoded, [-1, 4]), tf.reshape(rpn_label_logits, [-1]), image_shape2d,
        ...
        if is_training:
            ...
            # rpn loss
            all_losses.extend(
                    rpn_losses(anchors.gt_labels, anchors.encoded_gt_boxes(), rpn_label_logits, rpn_box_logits))
            ...
            if cfg.MODE_MASK:
                ...
                mask_logits = maskrcnn_upXconv_head('maskrcnn', feature_maskrcnn, cfg.DATA.NUM_CATEGORY,
...
class ResNetFPNModel(DetectionModel):
    ...
    def inputs(self):
        ...
        for k in range(len(cfg.FPN.ANCHOR_STRIDES)):
            ret.extend([
                    tf.placeholder(tf.int32, (None, None, num_anchors), 'anchor_labels_lvl{}'.format(k + 2)),
                    tf.placeholder(tf.float32, (None, None, num_anchors, 4), 'anchor_boxes_lvl{}'.format(k + 2))
            ])
        ret.extend([tf.placeholder(tf.float32, (None, 4), 'gt_boxes'),
    ...
    def build_graph(self, *inputs):
        ...
        multilevel_anchors = [
                RPNAnchors(all_anchors_fpn[i], inputs['anchor_labels_lvl{}'.format(i + 2)],
        ...
        rpn_outputs = [rpn_head('rpn', pi, cfg.FPN.NUM_CHANNEL, len(cfg.RPN.ANCHOR_RATIOS)) for pi in p23456]
        multilevel_label_logits = [k[0] for k in rpn_outputs]
        multilevel_box_logits = [k[1] for k in rpn_outputs]

        proposal_boxes, proposal_scores = generate_fpn_proposals(multilevel_anchors, multilevel_label_logits,
        ...
        if not cfg.FPN.CASCADE:
            ...
            fastrcnn_label_logits, fastrcnn_box_logits = fastrcnn_outputs('fastrcnn/outputs', head_feature,
                                                                          cfg.DATA.NUM_CLASS)
            fastrcnn_head = FastRCNNHead(proposals, fastrcnn_box_logits, fastrcnn_label_logits,
                                         tf.constant(cfg.FRCNN.BBOX_REG_WEIGHTS, dtype=tf.float32))
        else:
            ...
            fastrcnn_head = CascadeRCNNHead(proposals, roi_func, fastrcnn_head_func, image_shape2d, cfg.DATA.NUM_CLASS)

        if is_training:
            all_losses = []
            all_losses.extend(multilevel_rpn_losses(multilevel_anchors, multilevel_label_logits, multilevel_box_logits))
            ...
            if cfg.MODE_MASK:
                ...
                roi_feature_maskrcnn = multilevel_roi_align(
                        p23456[:4], proposals.fg_boxes(), 14, name_scope='multilevel_roi_align_mask')
                maskrcnn_head_func = getattr(model_mrcnn, cfg.FPN.MRCNN_HEAD_FUNC)
                mask_logits = maskrcnn_head_func('maskrcnn', roi_feature_maskrcnn,
    ...
    with tqdm.tqdm(total=nr_visualize) as pbar:
        for idx, dp in itertools.islice(enumerate(df), nr_visualize):
            ...
            results = [
                    ...
                    for args in zip(final_boxes, final_scores, final_labels, [None] * len(final_labels))
            ]
...
class EvalCallback(Callback):
    ...
    def _setup_graph(self):
        ...
        if cfg.TRAINER == 'replicated':
            ...
            self.dataflows = [
                    get_eval_dataflow(dataset=cfg.DATA.VAL_PATTERN),
                    get_eval_dataflow(dataset=cfg.DATA.TEST_PATTERN)
            ]
        else:
    ...
    def _eval(self):
        ...
        if cfg.TRAINER == 'replicated':
            ...
            with ThreadPoolExecutor(max_workers=self.num_predictor) as executor, \
                ...
                all_results = list(itertools.chain(*[fut.result() for fut in futures[:1]]))
                all_results_test = list(itertools.chain(*[fut.result() for fut in futures[1:2]]))
        else:
            if self._horovod_run_eval:
                local_results = eval_coco(self.dataflow, self.predictor)
                output_partial = os.path.join(logdir, 'outputs{}-part{}.json'.format(
            ...
            for k in range(hvd.local_size()):
                output_partial = os.path.join(logdir, 'outputs{}-part{}.json'.format(self.global_step, k))
                with open(output_partial, 'r') as f:
                    obj = json.load(f)
                all_results.extend(obj)
                os.unlink(output_partial)

        output_file = os.path.join(logdir, 'outputs{}.json'.format(self.global_step))
        with open(output_file, 'w') as f:
            json.dump(all_results, f)
        try:
        ...
        output_file_test = os.path.join(logdir, 'outputs-test{}.json'.format(self.global_step))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--logdir', help='log directory', default='data_out/maskrcnn')
    ...
    parser.add_argument(
            '--predict', help="Run prediction on a given image. "
    ...
    if args.visualize or args.evaluate or args.predict:
        ...
        callbacks = [
                PeriodicCallback(ModelSaver(max_to_keep=10, keep_checkpoint_every_n_hours=1), every_k_epochs=5),
                # linear warmup
