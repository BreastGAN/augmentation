...
def resnet_shortcut(layer, n_out, stride, activation=tf.identity):
    n_in = layer.shape[1]
    if n_in != n_out:  # change dimension when channel is not the same
        ...
        if not cfg.MODE_FPN and stride == 2:
            layer = layer[:, :, :-1, :-1]
        return Conv2D('convshortcut', layer, n_out, 1, strides=stride, activation=activation)
    else:
        return layer
...
def resnet_bottleneck(layer, ch_out, stride):
    ...
    if cfg.BACKBONE.STRIDE_1X1:
        ...
        if stride == 2:
            layer = tf.pad(layer, [[0, 0], [0, 0], maybe_reverse_pad(0, 1), maybe_reverse_pad(0, 1)])
            layer = Conv2D('conv2', layer, ch_out, 3, strides=2, padding='VALID')
        else:
            layer = Conv2D('conv2', layer, ch_out, 3, strides=stride)
    layer = Conv2D('conv3', layer, ch_out * 4, 1, activation=get_norm(zero_init=True))
    ret = layer + resnet_shortcut(shortcut, ch_out * 4, stride, activation=get_norm(zero_init=False))
    return tf.nn.relu(ret, name='output')


def resnet_group(name, layer, block_func, features, count, stride):
    with tf.variable_scope(name):
        for i in range(0, count):
            with tf.variable_scope('block{}'.format(i)):
                layer = block_func(layer, features, stride if i == 0 else 1)
    return layer
...
def resnet_c4_backbone(image, num_blocks):
    ...
    with backbone_scope(freeze=freeze_at > 0):
        layer = tf.pad(image, [[0, 0], [0, 0], maybe_reverse_pad(2, 3), maybe_reverse_pad(2, 3)])
        layer = Conv2D('conv0', layer, 64, 7, strides=2, padding='VALID')
        layer = tf.pad(layer, [[0, 0], [0, 0], maybe_reverse_pad(0, 1), maybe_reverse_pad(0, 1)])
        layer = MaxPooling('pool0', layer, 3, strides=2, padding='VALID')

    with backbone_scope(freeze=freeze_at > 1):
        c2 = resnet_group('group0', layer, resnet_bottleneck, 64, num_blocks[0], 1)
...
def resnet_conv5(image, num_block):
    with backbone_scope(freeze=False):
        layer = resnet_group('group3', image, resnet_bottleneck, 512, num_block, 2)
        return layer
...
def resnet_fpn_backbone(image, num_blocks):
    ...
    with backbone_scope(freeze=freeze_at > 0):
        ...
        layer = tf.pad(
                image,
                tf.stack([[0, 0], [0, 0], [pad_base[0], pad_base[1] + pad_shape2d[0]],
                          [pad_base[0], pad_base[1] + pad_shape2d[1]]]))
        layer.set_shape([None, chan, None, None])
        layer = Conv2D('conv0', layer, 64, 7, strides=2, padding='VALID')
        layer = tf.pad(layer, [[0, 0], [0, 0], maybe_reverse_pad(0, 1), maybe_reverse_pad(0, 1)])
        layer = MaxPooling('pool0', layer, 3, strides=2, padding='VALID')
    with backbone_scope(freeze=freeze_at > 1):
