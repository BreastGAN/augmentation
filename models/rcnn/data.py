...
from tensorpack.dataflow import (imgaug, TestDataSpeed, MultiProcessMapDataZMQ, MultiThreadMapData, MapDataComponent,
                                 DataFromList)
...
from models.rcnn.utils.generate_anchors import generate_anchors
from models.rcnn.utils.np_box_ops import area as np_area
from models.rcnn.common import (DataFromListOfDict, CustomResize, filter_boxes_inside_shape, box_to_point8,
...
def get_rpn_anchor_input(im, boxes, is_crowd):
    ...
    assert len(is_crowd.shape) == 1 and is_crowd.shape[0] == boxes.shape[0]
    boxes = boxes.copy()
...
def get_multilevel_rpn_anchor_input(im, boxes, is_crowd):
    ...
    for level_anchor in anchors_per_level:
        ...
        multilevel_inputs.append((all_labels[start:end].reshape(anchor_shape),
...
def get_train_dataflow():
    ...
    roidbs = BreastDetection.load_many(cfg.DATA.TRAIN_PATTERN, shuffle_dataset=True)
    """
...
def get_eval_dataflow(shard=0, num_shards=1, dataset=None):
    """
    Args:
        shard, num_shards: to get subset of evaluation data
    """
    if not dataset:
        dataset = cfg.DATA.VAL_PATTERN
    roidbs = BreastDetection.load_many(dataset, shuffle_dataset=False)
    num_imgs = len(roidbs)
