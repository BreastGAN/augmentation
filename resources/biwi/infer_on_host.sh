...
echo "Run model:"
cd $PROJECT_HOME
base_path="/scratch_net/$dataserver/$USER/transformed/small_all_256x204_final"
exec $VIRTUAL_ENV_PATH/bin/python -m models.breast_cycle_gan.inference --height 256 --width 204 --image_x_file "$base_path/healthy.$split.tfrecord" --image_y_file "$base_path/cancer.$split.tfrecord" --include_masks="$masks" --checkpoint_path="$checkpoint_path" --use_icnr="$icnr" --upsample_method="$upsample_method" --loss_identity_lambda="$loss_identity_lambda" --use_spectral_norm="$spectral_norm" --generated_dir="$output_dir" --batch_size=5
