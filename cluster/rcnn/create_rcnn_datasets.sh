...
in_data="$scratch/transformed/small_all_512x408_final"
data="$scratch/mammography/data_out"
r_data="$scratch/rcnn"

inbreast_corrections="--inbreast_corrections=$scratch/mammography/resources/data/inbreast_ground_truth_cancer_bbox_rois.tsv"
...
for stage in "train" "eval" "test"; do
    ...
    qsub ./create_rcnn_dataset.sh "$r_data/orig_mask/*.${stage}.tfrecord" "$r_data/orig_mask_rcnn/${stage}.tfrecord" "" "" "$test_corrections"
    qsub ./create_rcnn_dataset.sh "$r_data/gan_mask/*.${stage}.tfrecord" "$r_data/gan_mask_rcnn/${stage}.tfrecord" "--convert_generated" "$train_flip" "$test_corrections"
    qsub ./create_rcnn_dataset.sh "$r_data/gan_nomask/*.${stage}.tfrecord" "$r_data/gan_nomask_rcnn/${stage}.tfrecord" "--convert_generated" "$train_flip" "$test_corrections"
    qsub ./create_rcnn_dataset.sh "$r_data/aug_mask/*.${stage}.tfrecord" "$r_data/aug_mask_rcnn/${stage}.tfrecord" "--convert_generated" "" "$test_corrections"
done
